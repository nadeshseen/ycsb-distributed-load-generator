 ...
 * proportion of different kinds of operations, and other properties of the
 * workload, are controlled
...
public class ReplayWorkload extends Workload {
	...
	// Sending signal to the python server

	public boolean down_call(String start_signal_sender_ip, String operation, String key) {
		try {
			System.out.println(" " + start_signal_sender_ip + " " + operation);
			URL url = new URL("http://localhost:5000/down/");
			HttpURLConnection conn = (HttpURLConnection) url.openConnection();
			conn.setDoOutput(true);
			conn.setRequestMethod("POST");
			conn.setRequestProperty("Content-Type", "application/json");
			// "worker_node_username":"seen2",
			// "curr_machine_ip" : "192.168.122.111",
			String input = "{\"start_signal_sender_ip\":\"" + start_signal_sender_ip +"\", \"operation\":\"" + operation+ "\", \"key\":\"" + key +"\"}";

			OutputStream os = conn.getOutputStream();
			os.write(input.getBytes()); // input in bytes in os
			os.flush();
	...
	public boolean up_call(String send_here, String operation, String key) {
		try {
			System.out.println(send_here + " " + operation);
			URL url = new URL("http://localhost:5000/up/");
			HttpURLConnection conn = (HttpURLConnection) url.openConnection();
			conn.setDoOutput(true);
			conn.setRequestMethod("POST");
			conn.setRequestProperty("Content-Type", "application/json");
			// "worker_node_username":"seen2",
			// "sender_machine_ip" : "192.168.122.111",
			String input = "{\"send_here\":\"" + send_here + "\", \"operation\":\"" + operation	+"\", \"key\":\"" + key +"\"}";
	...
	public boolean doTransaction(DB db, Object threadstate) {
		...
		if (op.compareTo("READ") == 0) {
			if (trace.length >= 5) {
				...
				String str_val = trace[4];

				System.out.println(str_val);
				if (str_val.compareTo("WAIT_SIGNAL") == 0) {
					int num_of_waits = trace.length-5;
					System.out.println(num_of_waits);
					for(int i=0;i<num_of_waits;i++){
						String start_signal_sender_ip = trace[5+i];
						// String curr_machine_ip = trace[6];
						down_call(start_signal_sender_ip, str_val, dbkey);
			...
			if (trace.length >= 5) {
				...
				String str_val = trace[4];
				if (str_val.compareTo("START_SIGNAL") == 0) {

					System.out.println(str_val);
					String send_here = trace[5];
					up_call(send_here, str_val, dbkey);
			...
			if (trace.length >= 5) {
				...
				if (str_val.compareTo("START_SIGNAL") == 0) {
					...
					System.out.println(str_val);
					// String sender_machine_ip = trace[5];
					String send_here = trace[5];
					up_call(send_here, str_val, dbkey);
			...
			if (trace.length >= 5) {
				...
				System.out.println(str_val);
				if (str_val.compareTo("WAIT_SIGNAL") == 0) {
					String start_signal_sender_ip = trace[5];
					// String curr_machine_ip = trace[6];
					down_call(start_signal_sender_ip, str_val, dbkey);
				}
			}
